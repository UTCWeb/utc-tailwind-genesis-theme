...
You'll see output highlighting issues with PHP files that do not conform to UTC coding standards.
...
- `npm run makepot` to regenerate the `languages/utc.pot` file.
...
- `npm run zip` to create a utc.zip. Files in the `excludes` array in `scripts/makezip.js` are omitted.
...
4. Type `npm run zip` to create `utc.zip`. Files in the `excludes` array in `scripts/makezip.js` are omitted from the zip. `filename.md` files will be renamed to `filename.txt`.
