 ...
 * UTC.
 *
 * This file adds the WooCommerce styles and the Customizer additions for WooCommerce to the UTC Theme.
 *
 * @package UTC
 * @author  StudioPress
...
add_filter( 'woocommerce_enqueue_styles', 'utc_woocommerce_styles' );
/**
 ...
 */
function utc_woocommerce_styles( $enqueue_styles ) {

	$enqueue_styles[ genesis_get_theme_handle() . '-woocommerce-styles' ] = [
		'src'     => get_stylesheet_directory_uri() . '/lib/woocommerce/utc-woocommerce.css',
		'deps'    => '',
...
add_action( 'wp_enqueue_scripts', 'utc_woocommerce_css' );
/**
...
function utc_woocommerce_css() {
	...
	$color_link   = get_theme_mod( 'utc_link_color', $appearance['default-colors']['link'] );
	$color_accent = get_theme_mod( 'utc_accent_color', $appearance['default-colors']['accent'] );
	...
	',
		$color_accent,
		utc_color_contrast( $color_accent )
	) : '';
