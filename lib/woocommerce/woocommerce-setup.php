 ...
 * UTC.
 *
 * This file adds the required WooCommerce setup functions to the UTC Theme.
 *
 * @package UTC
 * @author  StudioPress
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'utc_woocommerce_breakpoint' );
/**
 ...
 */
function utc_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'utc_default_products_per_page' );
/**
 ...
 */
function utc_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'utc_woocommerce_pagination' );
/**
 ...
 */
function utc_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'utc' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'utc' ) );

	return $args;

}

add_action( 'after_switch_theme', 'utc_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function utc_woocommerce_image_dimensions_after_theme_setup() {
	...
	utc_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'utc_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function utc_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function utc_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'utc_gallery_image_thumbnail' );
/**
 ...
 */
function utc_gallery_image_thumbnail( $size ) {
